...
## Sidebar Design (Added 2026-02-28)
1. **Collapsible Sidebar**: Implement a collapsible sidebar with smooth transition animations.
2. **Icon-Only Mode**: When collapsed, only show icons with tooltips on hover.
3. **Toggle Button**: Add a toggle button on the sidebar to expand/collapse.
4. **Auto-Expand**: Automatically expand the sidebar when any menu item is clicked.
5. **Responsive Design**: Ensure the sidebar works well on different screen sizes.

## Layout Structure (Added 2026-02-28)
1. **Standard Layout**: Use the following layout structure for all pages:
   - Left sidebar (collapsible)
   - Top navbar (with breadcrumb and user dropdown)
   - Main content area (with app-container)
2. **App Container**: Use `.app-container` class for main content with proper padding and shadow.
3. **Responsive Breakpoints**: Ensure layout adapts to different screen sizes.

## Component Usage Guidelines (Added 2026-02-28)
1. **Element UI Components**: Use Element UI components consistently across the application.
2. **Form Design**: Use `el-form` with proper label widths and layout.
3. **Table Design**: Use `el-table` with appropriate column widths and responsive behavior.
4. **Dialog Usage**: Use `el-dialog` for modal dialogs with proper sizing and positioning.
5. **Steps Component**: Use `el-steps` for multi-step forms and processes.

## Performance Optimization (Added 2026-02-28)
1. **Lazy Loading**: Implement lazy loading for components when possible.
2. **Code Splitting**: Split code into smaller chunks for faster initial load times.
3. **Image Optimization**: Optimize images for web use to reduce load times.
4. **Caching**: Implement caching strategies for frequently used data.

## Accessibility (Added 2026-02-28)
1. **Keyboard Navigation**: Ensure all interactive elements are accessible via keyboard.
2. **Screen Reader Support**: Add appropriate ARIA attributes for screen reader support.
3. **Color Contrast**: Ensure sufficient color contrast for text readability.
4. **Focus Management**: Properly manage focus states for interactive elements.

## Security Considerations (Added 2026-02-28)
1. **Input Validation**: Validate all user inputs to prevent malicious data.
2. **Sanitization**: Sanitize user-generated content to prevent XSS attacks.
3. **Authentication**: Implement proper authentication mechanisms for protected routes.
4. **Authorization**: Ensure users only have access to appropriate resources.

## Deployment Best Practices (Added 2026-02-28)
1. **GitHub Pages**: Ensure all files are properly structured for GitHub Pages deployment.
2. **Static Assets**: Optimize static assets for production.
3. **Version Control**: Use git for version control with meaningful commit messages.
4. **Branch Management**: Use branches for feature development and bug fixes.

## Testing Guidelines (Added 2026-02-28)
1. **Cross-Browser Testing**: Test the application in multiple browsers.
2. **Responsive Testing**: Test the application on different screen sizes.
3. **Functional Testing**: Test all functional flows to ensure they work as expected.
4. **Usability Testing**: Test the application with real users to gather feedback.

## Documentation (Added 2026-02-28)
1. **Code Comments**: Add comments to explain complex code logic.
2. **README.md**: Maintain a comprehensive README.md file with project information.
3. **Design Documentation**: Keep design documentation up to date with current implementation.
4. **API Documentation**: Document any API endpoints used in the application.

## Maintenance (Added 2026-02-28)
1. **Code Quality**: Maintain high code quality with consistent coding standards.
2. **Bug Fixes**: Address bugs promptly and thoroughly.
3. **Feature Enhancements**: Implement new features in a structured and maintainable way.
4. **Technical Debt**: Regularly address technical debt to keep the codebase healthy.